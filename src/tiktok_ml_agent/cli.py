...
from .scale_baseline import (
    generate_scale_submission,
    generate_scale_submission_resumable,
    rescore_frozen_scale_model,
    run_streaming_popularity,
...
def main() -> None:
    ...
    autonomous_item_tab_history = commands.add_parser("autonomous-item-tab-history", help="run the strict-prior video×tab candidate cross")
    autonomous_item_tab_history.add_argument("--repository-root", type=Path, default=Path("."))
    autonomous_item_tab_history.add_argument("--starter-kit", type=Path, default=Path("kuairand-starter-kit"))
    ...
    autonomous_ensemble_confirmation = commands.add_parser("autonomous-ensemble-confirmation", help="run three single-vector frozen-ensemble confirmations")
    autonomous_ensemble_confirmation.add_argument("--repository-root", type=Path, default=Path("."))
    autonomous_ensemble_confirmation.add_argument("--starter-kit", type=Path, default=Path("kuairand-starter-kit"))
    autonomous_ensemble_confirmation.add_argument("--data-dir", type=Path, default=Path("kuairand-starter-kit/KuaiRand-Pure/data"))
    autonomous_ensemble_confirmation.add_argument("--leader-ledger", type=Path, default=Path("artifacts/autonomous-ensemble-revalidation/ledger.sqlite"))
    autonomous_ensemble_confirmation.add_argument("--bpr-ledger", type=Path, default=Path("artifacts/autonomous-ranking-verified/ledger.sqlite"))
    autonomous_ensemble_confirmation.add_argument("--history-ledger", type=Path, default=Path("artifacts/autonomous-history-verified/ledger.sqlite"))
    autonomous_ensemble_confirmation.add_argument("--temporal-ledger", type=Path, default=Path("artifacts/autonomous-temporal/ledger.sqlite"))
    autonomous_ensemble_confirmation.add_argument("--output-dir", type=Path, default=Path("artifacts/autonomous-ensemble-confirmation"))
    autonomous_ensemble_revalidation = commands.add_parser("autonomous-ensemble-revalidation", help="revalidate the frozen ensemble leader under the current data identity")
    autonomous_ensemble_revalidation.add_argument("--repository-root", type=Path, default=Path("."))
    autonomous_ensemble_revalidation.add_argument("--starter-kit", type=Path, default=Path("kuairand-starter-kit"))
    autonomous_ensemble_revalidation.add_argument("--data-dir", type=Path, default=Path("kuairand-starter-kit/KuaiRand-Pure/data"))
    autonomous_ensemble_revalidation.add_argument("--source-leader-ledger", type=Path, default=Path("artifacts/autonomous-three-ensemble/ledger.sqlite"))
    autonomous_ensemble_revalidation.add_argument("--bpr-ledger", type=Path, default=Path("artifacts/autonomous-ranking-verified/ledger.sqlite"))
    autonomous_ensemble_revalidation.add_argument("--history-ledger", type=Path, default=Path("artifacts/autonomous-history-verified/ledger.sqlite"))
    autonomous_ensemble_revalidation.add_argument("--temporal-ledger", type=Path, default=Path("artifacts/autonomous-temporal/ledger.sqlite"))
    autonomous_ensemble_revalidation.add_argument("--output-dir", type=Path, default=Path("artifacts/autonomous-ensemble-revalidation"))
    ...
    scale_resume_submission = commands.add_parser("scale-resume-submission", help="resume a feature-only scale submission from a frozen bounded model")
    scale_resume_submission.add_argument("--variant", choices=("1k", "27k"), required=True)
    scale_resume_submission.add_argument("--data-dir", type=Path, required=True)
    scale_resume_submission.add_argument("--model", type=Path, required=True)
    scale_resume_submission.add_argument("--output", type=Path, required=True)
    scale_resume_submission.add_argument("--state", type=Path, required=True, help="identity-gated progress-state JSON")
    scale_resume_submission.add_argument("--data-fingerprint", required=True, help="preflight fingerprint bound into the resume identity")
    scale_resume_submission.add_argument("--evaluator", type=Path, default=Path("kuairand-starter-kit/evaluate.py"), help="versioned evaluator identity to bind, never invoked for test scoring")
    scale_resume_submission.add_argument("--checkpoint-every", type=int, default=100_000)
    scale_resume_submission.add_argument("--restart", action="store_true", help="discard existing progress rather than resuming it")
    ...
    scale_resume_validation = commands.add_parser("scale-resume-validation", help="resume validation-only scoring of a frozen scale model")
    scale_resume_validation.add_argument("--variant", choices=("1k", "27k"), required=True)
    scale_resume_validation.add_argument("--data-dir", type=Path, required=True)
    scale_resume_validation.add_argument("--model", type=Path, required=True)
    scale_resume_validation.add_argument("--evaluator", type=Path, default=Path("kuairand-starter-kit/evaluate.py"))
    scale_resume_validation.add_argument("--state", type=Path, required=True, help="identity-gated progress-state JSON")
    scale_resume_validation.add_argument("--data-fingerprint", required=True, help="preflight fingerprint bound into the resume identity")
    scale_resume_validation.add_argument("--shards", type=int, default=256)
    scale_resume_validation.add_argument("--checkpoint-every", type=int, default=10_000_000)
    scale_resume_validation.add_argument("--item-weight", type=float)
    scale_resume_validation.add_argument("--restart", action="store_true", help="discard existing progress rather than resuming it")
    ...
    if args.command == "qualification":
        ...
        config = RankingFMConfig(objective=args.objective, history_cross=args.history_cross, item_tab_history_cross=args.item_tab_history_cross)
    ...
    elif args.command == "autonomous-item-tab-history":
        print(
            json.dumps(
                run_autonomous_item_tab_history(
                    repository_root=args.repository_root,
                    starter_kit_dir=args.starter_kit,
                    data_dir=args.data_dir,
                    parent_ledger_path=args.parent_ledger,
    ...
    elif args.command == "autonomous-ensemble-confirmation":
        print(json.dumps(run_autonomous_ensemble_confirmation(repository_root=args.repository_root, starter_kit_dir=args.starter_kit, data_dir=args.data_dir, leader_ledger_path=args.leader_ledger, bpr_ledger_path=args.bpr_ledger, history_ledger_path=args.history_ledger, temporal_ledger_path=args.temporal_ledger, output_dir=args.output_dir), indent=2, sort_keys=True))
    elif args.command == "autonomous-ensemble-revalidation":
        print(json.dumps(run_autonomous_ensemble_revalidation(repository_root=args.repository_root, starter_kit_dir=args.starter_kit, data_dir=args.data_dir, source_leader_ledger_path=args.source_leader_ledger, bpr_ledger_path=args.bpr_ledger, history_ledger_path=args.history_ledger, temporal_ledger_path=args.temporal_ledger, output_dir=args.output_dir), indent=2, sort_keys=True))
    ...
    elif args.command == "scale-resume-submission":
        result = generate_scale_submission_resumable(
            model_path=args.model, variant=args.variant, data_dir=args.data_dir,
            output_path=args.output, state_path=args.state,
            data_fingerprint=args.data_fingerprint, evaluator_path=args.evaluator,
            checkpoint_every=args.checkpoint_every, resume=not args.restart,
        )
        print(json.dumps(result, indent=2, sort_keys=True))
        ...
        print(json.dumps(result, indent=2, sort_keys=True))
    elif args.command == "scale-resume-validation":
        result = score_scale_validation_resumable(
            model_path=args.model, variant=args.variant, data_dir=args.data_dir,
            evaluator_path=args.evaluator, state_path=args.state,
            data_fingerprint=args.data_fingerprint, shards=args.shards,
            checkpoint_every=args.checkpoint_every, item_weight=args.item_weight,
