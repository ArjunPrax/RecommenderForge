...
class CampaignEvidenceTests(unittest.TestCase):
    ...
    def test_eligible_contract_status_is_labelled_in_designated_final(self) -> None:
        for contract_status in ("team_interpreted", "confirmed"):
            with self.subTest(contract_status=contract_status), tempfile.TemporaryDirectory() as directory:
                root = Path(directory)
                for index, score in enumerate((0.601, 0.6015, 0.6014), start=1):
                    _ledger_with_run(root / f"run-{index}.sqlite", f"run-{index}", score)
                config = {
                    "campaign_id": "fixture", "benchmark_id": "kuairand-pure", "contract_status": contract_status,
                    "baseline_primary": 0.60, "epsilon": 0.002, "patience": 3,
                    "batches": [
                        {"batch_id": f"b{index}", "runs": [{"ledger": f"run-{index}.sqlite", "run_id": f"run-{index}"}]}
                        for index in range(1, 4)
                ...
                self.assertEqual(final["designation_status"], contract_status)
                ledger = ExperimentLedger(final["final_ledger"])
                try:
                    record = ledger.get_run(final["final_run_id"])
                finally:
                    ledger.close()
                self.assertEqual(record["diagnosis"]["designation_status"], contract_status)
